...
export const flatSchema = {
  ...
  min: { value: 1, message: 'Flat number must be a positive number' }
};

export const floorSchema = {
  required: 'Floor number is required',
  min: { value: 1, message: 'Floor number must be a positive number' }
};

export const buildingSchema = {
  required: 'Building number is required',
  min: { value: 1, message: 'Building number must be a positive number' }
};
...
export const userSchema = yup.object().shape({
  usr_name: yup.string().required("First name is required"),
  usr_last_name: yup.string().required("Last name is required"),
  usr_email: yup
    .string()
    .email("Invalid email")
    .required("Email is required"),
