...
function NavCollapseLoop({ item, level }) {
  return item.children?.map((item) => {
    switch (item.type) {
      case 'collapse':
        return <NavCollapsePage key={item.id} item={item} level={level + 1} />;
      case 'item':
        return <NavItemPage key={item.id} item={item} level={level + 1} />;
      default:
        return (
...
export default function NavCollapsePage({ item, level }) {
...
NavCollapsePage.propTypes = { item: PropTypes.any, level: PropTypes.number };
