...
import { transactionList, lockersDetails as getLockerDetails } from "../../api/auth.api";
...
export default function SamplePage() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try { 
        const res = await transactionList(locker_id);
        const data =
          res?.data?.data ||
          res?.data ||
  ...
  const sortedRows = React.useMemo(() => {
    return usersData
      .filter(row => {
        ...
        (!filters.building || String(row.building_no) === String(filters.building)) &&
        (!filters.floor || String(row.floor_no) === String(filters.floor)) &&
        (!filters.flat || String(row.flat_no) === String(filters.flat)) &&
    ...
    <Box>
        <Box sx={{ padding: '0px 10px 10px' }}>
          <Typography color="text.primary" gutterBottom variant="h4">
            Transactions of Locker Code  : {lockersDetails?.locker_code}
          </Typography>
          ...
          <Grid size={{ xs: 12, sm: 6, lg: 2 }} sx={{ padding: '10px 10px',borderRadius: 2, textAlign: 'center',colo:'black' }}>
            ...
            <MenuItem value="" disabled>
              Building No
                ...
                <MenuItem key='1' value='1' sx={{ textTransform: 'capitalize' }}>
                  101
                </MenuItem>
                <MenuItem key='2' value='2' sx={{ textTransform: 'capitalize' }}>
                  102
                </MenuItem>
            </TextField> */}
          </Grid> 
